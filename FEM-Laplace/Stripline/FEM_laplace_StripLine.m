...
load stripline_mesh.mat     %load mesh data generated from pdetool
...
%% Assign the nodes on the dirichlet boundaries from e file and the mesh figure
      ...
      edge_D = [edge(1:30,1);edge(30,2);      % Dirichlet boundary with phi = 0
                edge(31:41,1);edge(41,2)];    % Dirichlet boundary with phi = 1000 V

%Assign the potential on the dirichlet boundaries
phi_D = zeros(size(edge_D,1),1);
for i = 32:size(edge_D,1)
     phi_D(edge_D(i)) = 1000;       %High potential on the trace
end

%% Solve phi_unknown;
phi_unknown = - L(44:end,44:end)\(L(44:end,1:43)* phi_D);  % Solve phi_unknown
...
trisurf(N,co(:,1),co(:,2),phi_all,'facecolor','interp')    
zlabel('Potential (V)')
colorbar
ax = gca;
ax.FontSize = 24;
...
% contourf(X,Y,phi_FEM_contour,20)
% colorbar
% hold on

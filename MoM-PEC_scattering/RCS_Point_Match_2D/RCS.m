function[Z_TM,I_TM,RCS_TM,Z_TE,I_TE,RCS_TE,phi]=RCS(a,N,phi_i)
Ns          =   1e3;
j           =   sqrt(-1);
eta0        =   120*pi;
k           =   2*pi;
...
Nss         =   50;
phi_s       =   linspace(0,pi,Nss);
plot(phi_s*180/pi,10*log10(PEC_RCS_TM(a,phi_s)),'ok','MarkerSize',3,'MarkerFaceColor','black','LineWidth',1)
plot(phi_s*180/pi,10*log10(PEC_RCS_TM(a,phi_s)),'ok')
hold off
legend('MM','Exact','Interpreter','latex','FontSize',15)
xlabel('$\varphi$ [deg]','Interpret','Latex','FontSize',15)
ylabel('$\sigma/\lambda$ [dB]','Interpret','Latex','FontSize',15)
set(gca,'TickLabel','Latex','FontSize',15)
title('RCS for TM','Interpret','Latex','FontSize',15)
xlim([0 180])
%%
figure()
err         =   abs(sigma-PEC_RCS_TM(a,phi))./abs(PEC_RCS_TM(a,phi));
plot(phi*180/pi,err,'k','LineWidth',1)
xlabel('$\varphi$ [deg]','Interpret','Latex','FontSize',15)
ylabel('$|\textrm{err}|$','Interpret','Latex','FontSize',15)
set(gca,'TickLabel','Latex','FontSize',15)
title('Relative error for TM','Interpret','Latex','FontSize',15)
xlim([0 180])
set(gca,'YScale','log')
ylim([1E-6 1E0])
%% TE Solution
...
Nss         =   50;
phi_s       =   linspace(0,pi,Nss);
plot(phi_s*180/pi,10*log10(PEC_RCS_TE(a,phi_s)),'ok','MarkerSize',3,'MarkerFaceColor','black','LineWidth',1)
plot(phi_s*180/pi,10*log10(PEC_RCS_TE(a,phi_s)),'ok')
hold off
legend('MM','Exact','Interpreter','latex','FontSize',15)
xlabel('$\varphi$ [deg]','Interpret','Latex','FontSize',15)
ylabel('$\sigma/\lambda$ [dB]','Interpret','Latex','FontSize',15)
set(gca,'TickLabel','Latex','FontSize',15)
title('RCS for TE','Interpret','Latex','FontSize',15)
xlim([0 180])
%%
figure()
err         =   abs(sigma-PEC_RCS_TE(a,phi))./abs(PEC_RCS_TE(a,phi));
plot(phi*180/pi,err,'k','LineWidth',1)
xlabel('$\varphi$ [deg]','Interpret','Latex','FontSize',15)
ylabel('$|\textrm{err}|$','Interpret','Latex','FontSize',15)
set(gca,'TickLabel','Latex','FontSize',15)
title('Relative error  for TE','Interpret','Latex','FontSize',15)
xlim([0 180])
set(gca,'YScale','log')
ylim([1E-6 1E0])
%%
...
%% Data columns: xn yn xn+1 yn+1 ln phi_n nx ny
Data        =   zeros(N,2);
for i=1:N
    xn          =   a*cos((2*pi/N)*(i-1));
    yn          =   a*sin((2*pi/N)*(i-1));
